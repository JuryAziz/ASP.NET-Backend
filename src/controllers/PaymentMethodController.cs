...
public class PaymentMethodsController(PaymentMethodService paymentMethodService) : ControllerBase
{
    ...
    public async Task<IActionResult> GetPaymentMethods([FromQuery] int page = 1, [FromQuery] int limit = 20)
    {
        try
        {
            ...
            return Ok(new SuccessResponse<IEnumerable<PaymentMethodModel>>() { Message = $"PaymentMethod fetched! page: {page}  limit: {limit}", Data = paymentMethods });
        }
        catch (Exception ex)
        {
    ...
    public async Task<IActionResult> GetPaymentMethodById(string paymentMethodId)
    {
        try
        {
            ...
            return Ok(new SuccessResponse<PaymentMethodModel>() { Message = $"PaymentMethod found with Guid {foundPaymentMethod.PaymentMethodId}", Data = foundPaymentMethod });
    ...
    public async Task<IActionResult> UpdatePaymentMethod(string paymentMethodId, PaymentMethodModel paymentMethod)
    {
        try
        {
            ...
            return Ok(new SuccessResponse<PaymentMethodModel>() { Message = $"User updated with guid of {paymentMethodToUpdate.PaymentMethodId}", Data = paymentMethodToUpdate });
        }
        catch (Exception ex)
        {
    ...
    public async Task<IActionResult> DeletePaymentMethod(string paymentMethodId)
    {
        try
        {
            ...
            return Ok(new SuccessResponse<PaymentMethodModel>() { Message = $"User deleted with the guid of {paymentMethodToDelete.PaymentMethodId}", Data = paymentMethodToDelete });
        }
        catch (Exception ex)
        {
